...
```{r}
rm <- rma(smd_ma, vd_ma, data = df, method = "DL") # DerSimonian-Laird is the default between-study heterogeneity estimator in the CMA software
rm
```
...
```{r}
rm <- rma(smd_ma, vd_ma, weights = weights_ma, data = df, method = "DL")
rm
```
...
```{r}
rm <- rma(smd_r_d, vd_r, data = df, method = "DL")
rm
```
