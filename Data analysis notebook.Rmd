...
# library(groundhog)
# pkgs <- c("readxl","writexl", "tidyverse", "metafor", "MAd")
# date = "2021-09-10"
...
1. Using the SMDs reproduced Strictly following information given in the meta-analysis or a standard procedure apparently adopted for at least 2 other primary SMDs (e.g., Cohen’s $d$ of tDCS vs. Sham at post intervention).
...
17. irreproducible. 
18. irreproducible.
19. successfully reproduced but not according to the information given in the MA. the values used are for changed in JHFT time from baseline whereas the meta-analysts report using "Time in JHFT".
...
```{r}
df <- transform(df, irrep_1_sec_dec = ifelse(round(smd_re1, digits = 2) == smd_ma, 1, 0))
sum(df$irrep_1_sec_dec, na.rm = TRUE)
sum(df$irrep_1_sec_dec, na.rm = TRUE)/21 * 100
```
#### reproducible to the second decimal place
```{r}
df <- transform(df, irrep_1_fir_dec = ifelse(round(smd_re1, digits = 1) == round(smd_ma, digits = 1), 1, 0))
sum(df$irrep_1_fir_dec, na.rm = TRUE)
sum(df$irrep_1_fir_dec, na.rm = TRUE)/21 * 100
```
...
#### Baujat 
```{r}
outl <- InfluenceAnalysis(ma3, random = TRUE)
plot(outl, "baujat")
```
...
Overview of the results of double-checking data extraction for each of the 20 primary SMDs:
1. SMD 1 could be reproduced using a $p$ value (given as a range <0.01) for the comparison between the control and treatment medians at post. It will be used for the pooled SMD. 
2. Best approximated by taking the first set of means and SDs (SMA) and doubling the sample size for the treatment group. It was negative because the variable is reaction times. The variation without doubling the sample size will be used.
3. Same as 1
4. Reproducible
...
# 2
df$smd_re1[[2]] <- compute_dgs(nt_ps, mt_ps.1, sdt_ps.1, nc_ps, mc_ps.1, sdc_ps.1, data = df[2,], denom = "pooled.sd")$g %>% abs()

# 3
df$smd_re1[[3]] <- NA

# 4
df$smd_re1[[4]] <- compute_dgs(nt_ps, mt_ps.1, sdt_ps.1, nc_ps, mc_ps.1, sdc_ps.1, data = df[4,], denom = "pooled.sd")$g
...
# 2
df$smd_re2[[2]] <- compute_dgs(nt_ps, mt_ps.1, sdt_ps.1, nc_ps, mc_ps.1, sdc_ps.1, data = df[2,], denom = "pooled.sd")$g %>% abs()
...
# 2
df$v_re1[[2]] <- compute_dgs(nt_ps, mt_ps.1, sdt_ps.1, nc_ps, mc_ps.1, sdc_ps.1, data = df[2,], denom = "pooled.sd")$var.g

# 3
df$v_re1[[3]] <- NA

# 4
df$v_re1[[4]] <- compute_dgs(nt_ps, mt_ps.1, sdt_ps.1, nc_ps, mc_ps.1, sdc_ps.1, data = df[4,], denom = "pooled.sd")$var.g
...
# 2
df$v_re2[[2]] <- compute_dgs(nt_ps, mt_ps.1, sdt_ps.1, nc_ps, mc_ps.1, sdc_ps.1, data = df[2,], denom = "pooled.sd")$var.g
