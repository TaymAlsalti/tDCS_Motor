...
df <- read_excel("data_thesis/Archive/Data_sheet_ps_ma1.xlsx")
```
...
### Functions for reproducing sampling variances
#### Compute variance of Cohen's $d$ for a between-groups comparison
To compute the pooled SMD, the sampling variance for each study, $\hat\sigma_i$, must be determined. The formula used here can be found in [Hedges and Olkin (1985, p. 86)](https://www.sciencedirect.com/book/9780080570655/statistical-methods-for-meta-analysis):
...
```{r}
# compute ds and corresponding sampling variances
...
At this point (07.09.21) I was reminded that, as in the case of the Cohen's $d$s themselves, the formula for computing the corresponding sampling variance wouldn't work for studies that had no control groups (cross-over trials). I contacted the authors of MA 1 on the 08.09.21 and received a reply that they used CMA to run the meta-analysis. As of 09.09.21, I'm waiting for further information.
...
df <- read_excel("data_thesis/Archive/Data_sheet_ps_ma1_updated.xlsx")
...
rm
```
By weighing the SMDs by the weights reported in the MA instead of the sampling variances calculated based on the reported CIs, the pooled SMD could be reproduced.
...
Since my data extraction procedure was somewhat different for MA2 and MA3 than for MA1, I did a final round of data extraction for MA1 so that my treatment is the same of all three meta-analyses. I used functions from the ` MAd` package because they reproduced mine.
...
1. MA1: Using the SMDs reproduced strictly following information given in the meta-analysis or a standard procedure apparently adopted for at least 2 other primary SMDs (e.g., Cohen’s $d$ of tDCS vs. Sham at post intervention).
